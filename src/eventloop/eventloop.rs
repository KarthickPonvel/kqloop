...
use crate::sys::{
    Event, Filter,
    unix::kqueue::{Kqueue, KqueueError},
};
...
impl EventLoop {
        ...
        W: FnMut(RawFd, &Event) -> Result<()> + Send + 'static,
    {
        self.register(
            fd,
            Filter::all(),
            Some(Box::new(read_cb)),
            Some(Box::new(write_cb)),
            None,
        )
    }
    ...
    pub fn deregister(&mut self, fd: RawFd, filter: Filter) -> Result<()> {
    ...
    fn poll(&mut self) -> Result<usize> {
        ...
        let timeout: Option<Duration> =
            match (self.config.poll_timeout_ms, self.next_timer_deadline()) {
                (Some(ms), Some(td)) => Some(td.min(Duration::from_millis(ms as u64))),
                (Some(ms), None) => Some(Duration::from_millis(ms as u64)),
                ...
                if event.filter.contains(Filter::READ) {
                    if let Some(ref mut cb) = handler.on_read {
                        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            ...
                            Err(_) => {
                                let err = EventLoopError::CallbackPanic(format!(
                                    "read callback panicked on fd {}",
                                    fd
                                ));
...
mod tests {
    ...
    fn test_event_loop_single_and_interval_timers() {
        ...
        assert_eq!(
            *single_counter.lock().unwrap(),
            1,
            "Single-shot timer should fire once"
        );
        assert!(
            *interval_counter.lock().unwrap() >= 3,
            "Interval timer should fire at least 3 times"
        );
    }
}
